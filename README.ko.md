...
**테스트.** `make test`는 `tests/`의 55개 테스트를 실행합니다. 여기에는 972건의
...
호스트: AMD Ryzen 7 5800H with Radeon Graphics, Linux 7.0.0-28-generic, odin version dev-2026-09-nightly:a2fb372, zig 0.16.0, rustc 1.98.1 (48a229cea 2026-09-01); Odin 빌드 옵션 `-o:speed -no-bounds-check -microarch:native`; `bench/results/latest.json`에 2026-09-16T14:40:24Z 기록.

커밋된 값 하나당 나노초, 인프로세스 전송, 직렬화 없음, 반복 표본의 중앙값 (낮을수록 좋음):

| 워크로드 | paxos-odin | paxos-zig | OmniPaxos | LibPaxos3 |
|---|---:|---:|---:|---:|
| 3 투표자, 8 B, 한 번에 하나 | 165 | 117 | 997 | 2,247 |
| 3 투표자, 8 B, 8개 동시 진행 | 149 | 117 | 196 | – |
| 3 투표자, 8 B, 64개 동시 진행 | 149 | 112 | 78 | – |
| 5 투표자, 8 B, 한 번에 하나 | 225 | 210 | 2,714 | – |
| 5 투표자, 8 B, 8개 동시 진행 | 197 | 206 | 445 | – |
| 3 투표자, 1 KiB, 한 번에 하나 | 486 | 2,700 | 1,244 | – |
| 3 투표자, 1 KiB, 8개 동시 진행 | 521 | 2,740 | 419 | – |
| 3 소유자, 8 B, 한 번에 하나, 순환 슬롯 소유 | 169 | – | – | – |
| 3 소유자, 8 B, 8개 동시 진행, 순환 슬롯 소유 | 161 | – | – | – |
...
|---|---|---:|---:|
| paxos-odin | 커밋 라운드마다 fsync, 값 하나 | 27.99 ms | 6.00 |
| paxos-odin | 커밋 라운드마다 fsync, 값 8개 | 3.82 ms | 0.75 |
| paxos-zig | fsync-each | 28.07 ms | – |
| paxos-zig | group8 | 3.65 ms | – |

있는 그대로 읽어야 합니다. 3 투표자, 8바이트 값에서는 paxos-zig가 값당 20%에서
30% 더 저렴하고, 5 투표자에서는 둘의 차이가 10분의 1 이내이며 8개를 동시에 진행할
때는 이 라이브러리가 앞서고, 1 KiB 값에서는 이 라이브러리가 값당 다섯 배 이상 더
저렴합니다. 값이 제안과 커밋 사이에 절대 복사되지 않기 때문입니다: 레코드와
...
- **소유 순서는 멤버십 순서입니다.** 순환 슬롯 소유 아래에서 슬롯 `s`는 정렬된
  멤버십 순서에서 `(s - 1) mod N`번째 멤버가 소유합니다. 정체된 접두는
...
├── tests/                   55개 테스트(odin test tests)와 공유 하네스
