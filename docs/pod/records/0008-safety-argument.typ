...
- *Lemma 14 (resubmission).* `record_commit` queues the owner's own losing suggestion only when the decided value differs; `drain_resubmits` proposes it in a later own slot through `propose_owned`. The queue holds one chunk; overflow relies on the host's retry.
...
- *Lemma 16 (contiguity).* `emit_contiguous` releases only `delivered_through + 1` while the cell is tagged and `.Chosen`; the pass-through in `record_commit` releases exactly `delivered_through + 1`; every released value is chosen. `learner_learn_chosen` does the same with `released_through`.
