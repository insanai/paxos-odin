...
#table(
  ...
  [High], [A new campaign retained an old leader proposal, which could reject the value required by the new quorum.], [Clear election/proposal state at the campaign boundary. `review_campaign_discards_prior_term_proposals`.],
  [High], [Moving between recovery chunks discarded previously observed chosen/trim boundaries.], [Preserve those boundaries across chunks and retain recovered evidence. `review_recovery_preserves_fences_across_chunks`.],
  [High], [Snapshot recovery erased votes above the snapshot prefix. Such votes can belong to an already chosen quorum.], [Preserve the durable suffix and promise; reset volatile leadership state. `review_snapshot_preserves_votes_above_anchor`.],
  [High], [Retry scans repeatedly started at the same slots. A six-slot recovery with two-slot chunks stalled after slot two.], [Rotate the bounded scan through the physical window. `review_multichunk_recovery_and_retry_progress` reproduced the stall and now passes.],
  [High], [A leader behind its followers skipped their decided slots during retry without fetching those decisions.], [Fetch an ahead peer's chosen prefix during retransmission. `review_leader_fetches_decisions_from_ahead_follower`; three-node partial-crash seed 77 reproduced the stall.],
  [High], [A locally committed stop sign was not observed until a later `step`; restoring a committed stop could reopen the epoch.], [Observe all decision-producing wrapper operations and durable restoration. `review_stop_seal_restore_and_completed_history`.],
  [High], [An out-of-order committed stop behind a gap disappeared from the pending-stop check.], [Committed and accepted stop entries both block further proposals. `review_out_of_order_chosen_stop_blocks_proposals` reproduced this gap.],
  ...
  [Medium], [Replaying a later slot could fail behind an accepted-only cell even when a durable trim certificate had closed the old slot.], [Allow reuse covered by the certified trim. `review_replay_reuses_certified_trimmed_vote` reproduced the failure.],
  ...
  [Medium], [Slot arithmetic could wrap in restoration, range construction, or learner advancement.], [Use checked/saturating bounds and terminate at exhaustion. `review_slot_exhaustion_and_learner_wrap`.],
  ...
  [Medium], [Live trim adoption ignored a conflicting record with the same trim ID.], [Reject conflicting identities and regressions before mutation. `review_live_trim_rejects_conflicting_identity`.],
  ...
  [Medium], [Promise range bounds and equal-ballot conflicting values were insufficiently checked.], [Validate the requested chunk and reject conflicting values. `review_promise_reordering_deduplication_and_validation`.],
...
- 55 `@(test)` procedures in `-debug` and `-o:speed` (`grep -c "@(test)" tests/*.odin`), including the five `ownership_*` scenarios, `review_thousand_voters_reach_quorum`, the bit-set tests, and the `election_matrix_preserves_chosen_values` enumeration of 972 cases;
...
`make bench-compare` (`tools/bench_compare.py`) reran this library, `paxos-zig`, OmniPaxos, and LibPaxos3 sequentially on the `0.2.0` sources and rewrote `bench/results/latest.json` (recorded 2026-09-16T14:40:24Z on the same AMD Ryzen 7 5800H host); the `0.1.0` figures quoted under "Four-way comparison" above survive only in that paragraph. The book's evidence chapter, the README, and the release notes read their tables from the recorded file. Against the `0.1.0` run, the 1 KiB workload, where the copies were the cost, fell from 1,091 ns to 486 ns per value one at a time; the three-voter 8-byte workload, where the transition logic is, moved from 145 ns to 165 ns; five voters from 185 ns to 225 ns. The file also carries the two `owned-3n` rows for rotating ownership (169 ns and 161 ns) and the durable rows (27.99 ms and 3.82 ms). POD 0009 states what the redesign was expected to change and what it was not.
