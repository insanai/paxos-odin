...
+ *Resubmission is at-least-once.* When the owner itself votes a revoker's accept, its suggestion is queued at the overwrite; if the revoker had re-proposed that same value (it found the vote in its read quorum), the slot decides it and the resubmission decides it again. Commands carry ids for this reason. Detecting the case (the revoker's value equals the suggestion) is straightforward but not done.
