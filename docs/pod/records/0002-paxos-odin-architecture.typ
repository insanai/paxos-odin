...
#table(
  ...
  [`membership.odin`], [`Membership(MAX_MEMBERS)` with its stable member index, the sorted `by_id` index, `LINEAR_LOOKUP_LIMIT`, and quorum validation (obligation B2).],
...
`Membership(MAX_MEMBERS)` holds `members` (a `small_array` of `Node_Id` in the caller's order; that position is the member's stable index in every per-member array of a node), `by_id` (the same set as `Member_Ref{id, index: u16}` sorted by id), `read_quorum_size`, and `write_quorum_size`. `membership_index_of` scans linearly for memberships of at most `LINEAR_LOOKUP_LIMIT = 8` members and binary-searches `by_id` above that. `membership_init` validates non-zero unique ids, defaults both quorums to a majority when the overrides are zero, and rejects `read + write <= total` with `.Non_Intersecting_Quorums` (obligation B2). Validation builds a local candidate and assigns it only on success, so a failed call leaves the caller's value untouched. `review_thousand_voters_reach_quorum` exercises the binary-search path; `review_hundred_twenty_eight_voters` the acknowledgement bitmap across a word boundary.
